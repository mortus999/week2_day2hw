...
# cormac_mccarthy = category["suspenseful"]
# category["suspenseful"] = {"Cormac Mccarthy": ["the road", "blood meridian", "no country for old men"], "other authors i know": "spy kids","mf doom" : "spy kids 2"}
# print(cormac_mccarthy)
# category["scary"]
# scary_books = category["scary"]
# category["scary"] = {"books": ["it", "the exorcist", "et"]}
# print(scary_books)

# category["action"]
# action_books = category["action"]
# category["action"]={"books": {"moby dick","the titanic","lost"}}
# print(action_books)
...
def shelf():
    book_shelf = {"Top Shelf": {"Trophies": "3rd grade"},
    "second shelf": {"books": "the name of the wind"}
}
    while True:
        response = input(f"choose : {add_catagory} {update} {display}:")
        if response == add_catagory:
            new_item = input("Which Shelf")
            item_defined = input("item: ")
            sub_item= input(f"{item_defined}: ")
            add_it = {item_defined : sub_item}
            book_shelf[new_item] = add_it 
            print(book_shelf)
            break
